...
var (
	Conf              config // holds the global app conf.
...
// then overriding settings from the app conf file, then overriding
...
func InitConfig(configFile string) error {
	...
	if _, err := os.Stat(configFile); err != nil {
		return errors.New("conf file err:" + err.Error())
	} else {
		log.Infof("load conf from file:" + configFile)
		configBytes, err := ioutil.ReadFile(configFile)
		if err != nil {
			return errors.New("conf load err:" + err.Error())
		}
		_, err = toml.Decode(string(configBytes), &Conf)
		if err != nil {
			return errors.New("conf decode err:" + err.Error())
		}
	}

	// @TODO 配置检查
	log.Infof("conf data:%v", Conf)
